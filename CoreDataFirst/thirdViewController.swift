...
class thirdViewController: UIViewController {
    ...
    @IBAction func save(_ sender: UIButton) {
        ...
        let entity = NSEntityDescription.entity(forEntityName: "Second", in: context)
        let newUser = NSManagedObject(entity: entity!, insertInto: context)
        newUser.setValue("\(username.text!)", forKey: "userName22")
        newUser.setValue("\(password.text!)", forKey: "password22")
        //save data
        do {
            try context.save()
        } catch {
